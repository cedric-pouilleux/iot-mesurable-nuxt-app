<template>
  <div class="">
    <div class="grid grid-cols-1 grid-rows-[100px_24px] gap-y-1">
      ...
      >
        ...
        >
          ...
          <div
            v-for="cat in tooltip.categories"
            :key="cat.name"
            class="flex items-center gap-1.5 my-0.5"
          >
            <span class="w-2 h-2 rounded-full" :style="{ background: cat.color }"></span>
            {{ cat.name }}: {{ cat.count }}
          </div>
          <div class="mt-1 pt-1 border-t border-gray-700 font-medium">
            Total: {{ formatNumber(tooltip.total) }}
          </div>
        </div>
      </div>
    ...
    <div
      v-if="selection"
      class="flex items-center justify-between px-4 py-1.5 bg-blue-50 dark:bg-blue-900/20 border-t border-blue-100 dark:border-blue-900/30 text-xs text-blue-800 dark:text-blue-200"
    >
      <span
        >Sélection: {{ formatDateTime(selection.start) }} →
        {{ formatDateTime(selection.end) }}</span
      >
      <button
...
const maxCount = computed(() => {
  ...
  return Math.max(...rawData.value.map(b => Object.values(b.counts).reduce((a, c) => a + c, 0)), 1)
})
  ...
  categories.forEach(cat => {
    ...
    for (let i = rawData.value.length - 1; i >= 0; i--) {
      ...
      if (i === rawData.value.length - 1) {
        ...
        const prevY =
          100 -
          (stackCurrent[i + 1] - ((rawData.value[i + 1].counts[cat] || 0) / maxCount.value) * 95)
...
const selectionRange = computed(() => {
  ...
  if (rawData.value.length > 1) {
    bucketDuration =
      new Date(rawData.value[1].slot).getTime() - new Date(rawData.value[0].slot).getTime()
  }
  ...
  try {
    ...
    if (props.filters.level) {
      props.filters.level
        .split(',')
        .filter(Boolean)
        .forEach(lvl => params.append('level', lvl))
    }
...
watch(() => [props.range, props.filters.category, props.filters.level], fetchHistogram, {
  deep: true,
  immediate: true,
})
</script>

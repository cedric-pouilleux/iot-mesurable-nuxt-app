...
export function useDeviceHealth(moduleId: Ref<string> | string): UseDeviceHealthReturn {
  ...
  const getSensorHealth = (sensorKey: string): SensorHealth | null => {
    ...
    return (
      cache.value.health.value.sensors.find(s => s.sensorType.endsWith(`:${sensorType}`)) || null
    )
  }
  ...
  const getConnectionStatus = (
    sensorKey: string
  ): 'connected' | 'stale' | 'offline' | 'unknown' => {
  ...
  onMounted(() => {
    ...
    const entry = cache.value
    if (!entry.lastFetch) {
      // A simple heuristic to check if it's the first time this module is mounted

...
describe('useHardwareSensorHistory', () => {
  ...
  it('should fallback to simple key when composite key not found', () => {
    ...
    const measureKey = ref('temperature')
    const sensorHistoryMap = ref({
      temperature: [{ time: new Date('2024-01-01T11:00:00Z'), value: 22.0 }],
    })
  ...
  it('should prefer composite key over simple key', () => {
    ...
    const measureKey = ref('temperature')
    const sensorHistoryMap = ref({
      'BME280:temperature': [{ time: new Date('2024-01-01T12:00:00Z'), value: 25.0 }],
      temperature: [{ time: new Date('2024-01-01T11:00:00Z'), value: 20.0 }],
    })
  ...
  it('should return last element from sorted history array', () => {
    ...
    const { lastUpdate } = useHardwareSensorHistory(hardwareKey, measureKey, sensorHistoryMap)
  ...
  it('should handle empty measure key', () => {
    ...
    const sensorHistoryMap = ref({
      'BME280:temperature': [{ time: new Date('2024-01-01T10:00:00Z'), value: 20.5 }],
    })
